...
const ForgotPassHost = () => {
  ...
  const passwordUpdate = async (e: React.FormEvent) => {
    ...
    try {
      const updatePassword = await hostAxios.post(API_URL.FORGOT_PASS_HOST, payload);
      if (updatePassword) {
        navigate(PUBLIC.HOST_ROUTE.FORGOT_PASS_OTP_HOST);
      }
    } catch (e) {
      console.log(e);
    }
