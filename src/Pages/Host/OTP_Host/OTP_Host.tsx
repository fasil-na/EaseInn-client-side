...
const OTP_Host = () => {
  ...
  const handleResendOTP = async () => {
    const response = await hostAxios.get(API_URL.RESEND_OTP_HOST);
    if (response) {
  ...
  const savedCountdownValue = Number(
    localStorage.getItem("hostCountdownValue")
  );
  const countdownEndTimestamp = Number(
    localStorage.getItem("hostCountdownEndTimestamp")
  );
  ...
  useEffect(() => {
    if (countdown > 0) {
      localStorage.setItem("hostCountdownValue", String(countdown));
      localStorage.setItem(
        "hostCountdownEndTimestamp",
        String(Date.now() + countdown * 1000)
      );
    } else {
      localStorage.removeItem("hostCountdownValue");
      localStorage.removeItem("hostCountdownEndTimestamp");
    }
  }, [countdown]);
  ...
  const verifyOTP = async (e: React.FormEvent) => {
    ...
    try {
      const response = await hostAxios.post(API_URL.VERIFY_OTP_HOST, payload);
      if (response.data.success) {
        localStorage.removeItem("hostCountdownValue");
        localStorage.removeItem("hostCountdownEndTimestamp");
        navigate(ROUTES.PUBLIC.HOST_ROUTE.LOG_IN);
      } else {
