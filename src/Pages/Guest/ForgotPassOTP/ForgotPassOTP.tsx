...
const ForgotPassOTP = () => {
  ...
  const handleResendOTP = async () => {
    const response = await guestAxios.get(API_URL.FORGOT_PASS_RESEND_OTP);
    if (response) {
  ...
  const savedCountdownValue = Number(
    localStorage.getItem("forgotPassCountdownValue")
  );
  const countdownEndTimestamp = Number(
    localStorage.getItem("forgotPassCountdownEndTimestamp")
  );
  ...
  useEffect(() => {
    if (countdown > 0) {
      localStorage.setItem("forgotPassCountdownValue", String(countdown));
      localStorage.setItem(
        "forgotPassCountdownEndTimestamp",
        String(Date.now() + countdown * 1000)
      );
    } else {
      localStorage.removeItem("forgotPassCountdownValue");
      localStorage.removeItem("forgotPassCountdownEndTimestamp");
    }
  }, [countdown]);
  ...
  const verifyOTP = async (e: React.FormEvent) => {
    ...
    try {
      const response = await guestAxios.post(
        API_URL.FORGOT_PASS_VERIFY_OTP,
        payload
      );
      if (response.data.success) {
        localStorage.removeItem("forgotPassCountdownValue");
        localStorage.removeItem("forgotPassCountdownEndTimestamp");
        navigate(ROUTES.PUBLIC.GUEST_ROUTE.RESET_PASS);
      } else {
