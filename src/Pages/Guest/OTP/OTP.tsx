...
const OTP = () => {
  ...
  const handleResendOTP = async () => {
    const response = await guestAxios.get(API_URL.RESEND_OTP);
    if (response) {
  ...
  const savedCountdownValue = Number(localStorage.getItem("countdownValue"));
  const countdownEndTimestamp = Number(
    localStorage.getItem("countdownEndTimestamp")
  );
  ...
  useEffect(() => {
    if (countdown > 0) {
      localStorage.setItem("countdownValue", String(countdown));
      ...
      localStorage.removeItem("countdownEndTimestamp");
    }
  }, [countdown]);
  ...
  const verifyOTP = async (e: React.FormEvent) => {
    ...
    try {
      const response = await guestAxios.post(API_URL.VERIFY_OTP, payload);
      if (response.data.success) {
        localStorage.removeItem("countdownValue");
        localStorage.removeItem("countdownEndTimestamp");
        navigate(ROUTES.PUBLIC.GUEST_ROUTE.LOG_IN);
      } else {
